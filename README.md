# ♾️ DeepSeekClaw — AI Inference You Own, Forever Powering Your OpenClaw Agents

**Open-source first.** DeepSeekClaw connects your [OpenClaw](https://github.com/openclaw/openclaw) agent to the [Morpheus](https://mor.org) decentralized inference network — putting open-source models like GLM-5 (Opus 4.5-level) front and center as your default, with Claude as a fallback only when needed.

> **DeepSeek-focused:** Optimized for DeepSeek models. Advanced reasoning, code generation, and extended thinking tasks.
...
```bash
clawhub install deepseekclaw-inference
```

**Or one-command installer:**

```bash
curl -fsSL https://raw.githubusercontent.com/profbernardoj/deepseekclaw.org/main/scripts/install-deepseekclaw.sh | bash
```

**Or manual clone:**

```bash
git clone https://github.com/profbernardoj/deepseekclaw.org.git ~/.openclaw/workspace/skills/deepseekclaw
```

> ⚠️ **Use `deepseekclaw-inference`** — not `deepseekclaw`. The bare `deepseekclaw` slug on ClawHub belongs to a different, unrelated product ("DeepSeekClaw Vault"). See [CLAWHUB_WARNING.md](CLAWHUB_WARNING.md).

Then configure your OpenClaw agent:

```bash
# Easiest: Gateway only (no local proxy needed)
node ~/.openclaw/workspace/skills/deepseekclaw/scripts/setup.mjs --template gateway-only --key YOUR_KEY --apply --test --restart

# Full: Local P2P + Gateway (auto-detects OS)
node ~/.openclaw/workspace/skills/deepseekclaw/scripts/setup.mjs --key YOUR_KEY --apply --test --restart
```
...
bash ~/.openclaw/workspace/skills/deepseekclaw/scripts/install.sh

# Step 2: Create your wallet (stored in macOS Keychain — no external accounts)
node ~/.openclaw/workspace/skills/deepseekclaw/scripts/deepseekclaw-wallet.mjs setup

# Step 3: Send ETH to the wallet address shown above, then swap for MOR
node ~/.openclaw/workspace/skills/deepseekclaw/scripts/deepseekclaw-wallet.mjs swap eth 0.05

# Step 4: Approve MOR for staking
node ~/.openclaw/workspace/skills/deepseekclaw/scripts/deepseekclaw-wallet.mjs approve

# Step 5: Install the proxy and guardian (auto-starts on boot)
bash ~/.openclaw/workspace/skills/deepseekclaw/scripts/install-proxy.sh
```
...
**You don't need to be an engineer to use DeepSeekClaw.** If you can copy and paste commands, you can set this up.

DeepSeekClaw is built for early adopters who:
...
If you've ever had your AI assistant go dark because an API key expired or credits ran out, DeepSeekClaw solves that problem permanently.
...
```
Your Agent → DeepSeekClaw Proxy (port 8083) → Morpheus P2P Network → AI Model
```
...
| Install (ClawHub) | `clawhub install deepseekclaw-inference` |
| Install (script) | `curl -fsSL https://raw.githubusercontent.com/profbernardoj/deepseekclaw.org/main/scripts/install-deepseekclaw.sh \| bash` |
| Update (ClawHub) | `clawhub update deepseekclaw-inference` |
| Update (git) | `cd skills/deepseekclaw && git pull` |
| Check version | `bash skills/deepseekclaw/scripts/install-deepseekclaw.sh --check` |
| Install router | `bash skills/deepseekclaw/scripts/install.sh` |
| Create wallet | `node scripts/deepseekclaw-wallet.mjs setup` |
| Check balance | `node scripts/deepseekclaw-wallet.mjs balance` |
| Swap ETH→MOR | `node scripts/deepseekclaw-wallet.mjs swap eth 0.05` |
| Swap USDC→MOR | `node scripts/deepseekclaw-wallet.mjs swap usdc 50` |
| Approve MOR | `node scripts/deepseekclaw-wallet.mjs approve` |
| Install proxy + guardian | `bash skills/deepseekclaw/scripts/install-proxy.sh` |
| Start router | `bash skills/deepseekclaw/scripts/start.sh` |
...
| **Always-on setup** | `sudo bash skills/deepseekclaw/scripts/always-on.sh` |
| Restore power defaults | `sudo bash skills/deepseekclaw/scripts/always-on.sh --restore` |
